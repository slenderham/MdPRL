function [loglikehood, V, A] = fMLchoiceLL_RL2conjobjdecayattn_constrained(xpar, sesdata)
%
...
alpha_rewft   = xpar([NparamBasic+1]) ;
alpha_rewconj = xpar([NparamBasic+2]) ;
alpha_rewobj  = xpar([NparamBasic+3]) ;
NparamWithLR = NparamBasic+2;
if sesdata.flagUnr==1
    alpha_unrft      = xpar([NparamBasic+4]) ;
    alpha_unrconj    = xpar([NparamBasic+5]) ;
    alpha_unrobj     = xpar([NparamBasic+6]) ;
    NparamWithLR = NparamBasic+4;
else
    alpha_unrft      = alpha_rewft ;
    alpha_unrconj    = alpha_rewconj ;
    alpha_unrobj     = alpha_rewobj ;
end
...
for cnt_trial=1:ntrials
    ...
    pChoiceR = 1./(1+exp(-(attn_w_choice(1)*(magF*(vf(idx_shape(2))-vf(idx_shape(1))) ...
                          ...
                          +magO*(vo(inputTarget(2, cnt_trial))-vo(inputTarget(1, cnt_trial)))) ...
                          +BiasL)) ;
