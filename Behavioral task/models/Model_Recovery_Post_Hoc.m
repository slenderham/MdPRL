...
%% simulating using best fit parameters
...
%             disp(strcat("Simulating subject ", num2str(cnt_samp)));
            inputname   = strcat("../PRLexp/inputs_all/", subjects_inputs(idxperf(cnt_samp)) , ".mat") ;
...
for m = 5
    ...
    for a = 1:length(attn_modes)
        ...
        for sim_m = 5
            for sim_a = 1:length(attn_modes)
                tic
                parfor cnt_samp = 1:length(idxperf)
...
cd ../files
save RPL2Analysis_Attention_model_recovery_by_attn_posthoc
cd ../models
