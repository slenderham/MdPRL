...
all_sim_par_samples = {...
    @() [max(min(randn(1)/5, 1.25), -1.25) ...
         min(exp(randn(1)+3), 500) ...
         unifrnd(0.001, 0.2, [1, 1]) ...
         unifrnd(0.01, 0.99, [1, 2])...
         exp(log(1)+rand(1)*(log(500)-log(1)))], ...
    @() [max(min(randn(1)/5, 1.25), -1.25) ...
         min(exp(randn(1)+3), 500) ...
         unifrnd(0.01, 0.99, [1, 1]) ...
         unifrnd(0.001, 0.2, [1, 1]) ...
         unifrnd(0.01, 0.99, [1, 2]) ...
         exp(log(1)+rand(1)*(log(500)-log(1)))], ...
    @() [max(min(randn(1)/5, 1.25), -1.25) ...
...
%             disp(strcat("Simulating subject ", num2str(cnt_samp)));
            ...
            while sim_acc < 0.53 || sim_ll > -logsigmoid(0.1)*trialLength
                % load model likelihood func and optimize
                ll = str2func(all_model_names(m));
                [fval, latents] = ll(ipars, sesdata);
                [~, idxMax] = max(inputs_struct.expr.prob{1}(input.inputTarget)) ;
                choiceRew = latents.C == idxMax ;
                sim_acc = mean(choiceRew(ntrialPerf));
                % make sure the model is not generating random or
                % repetitive choices
                sim_ll = sum(latents.pure_ll);
                ...
                tries = tries+1;
                if mod(tries, 1)==0
                    % if the model parameter is having trouble with
                    % performance, try sampling new parameters
                    ipars = all_sim_par_samples{m}();
                    ipars = ipars(1:NparamBasic+sesdata.Nalpha+sesdata.Nbeta);
                end
            end
...
%                         ubs(1:end-1) = ipar(1:end-1); pubs(1:end-1) = ipar(1:end-1);
                ...
                toc
                cd ../files
                save RPL2Analysis_Attention_model_recovery_by_input_thresh_both_1_500
                cd ../models
            end
        end
    end
