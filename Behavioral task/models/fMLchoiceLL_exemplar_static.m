...
attn_Shape = xpar(NparamWithLR+1) ;
attn_Color = xpar(NparamWithLR+2) ;
attn_Pattern = xpar(NparamWithLR+3) ;
...
for cnt_trial=1:ntrials
    ...
    if cnt_trial>1
        sims = [];
        sims(1,:,:) = [(idx_shape(1)~=stim_mem(:,1)) ...
                       (idx_color(1)~=stim_mem(:,2)) ...
                       (idx_pattern(1)~=stim_mem(:,3))];
        sims(2,:,:) = [(idx_shape(2)~=stim_mem(:,1)) ...
                       (idx_color(2)~=stim_mem(:,2)) ...
                       (idx_pattern(2)~=stim_mem(:,3))];

        sims = reshape(sims, [2, 3, cnt_trial-1]);
    
        % sims 2 X dimension X prev trials
    
        sims = sum(reshape([attn_Shape, attn_Color, attn_Pattern], [1,3,1]).*sims, 2);
