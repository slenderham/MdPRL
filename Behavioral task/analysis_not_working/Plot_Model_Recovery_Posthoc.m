...
recovery_results_by_input = load('../files/RPL2Analysis_Attention_model_recovery_by_input_posthoc.mat');
recovery_results_by_attn = load('../files/RPL2Analysis_Attention_model_recovery_by_attn_posthoc.mat');
...
for cnt_samp=1:67
    true_pars(cnt_samp+67,:) = recovery_results_by_attn.sim_results{5, 3, cnt_samp}.params;
    fit_pars(cnt_samp+67,:) = recovery_results_by_attn.fit_results{5, 3, cnt_samp, 5, 3}.params;
end
...
for a=1
    [alpha, ~, xp, pxp, bor, g] = bms(-BIC_temp_by_attn(:,:,a)'/2, mat2cell((1:10)', repmat([1], 1, 10)));
    disp(bor)
