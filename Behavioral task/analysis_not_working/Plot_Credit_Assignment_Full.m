...
all_var_names = all_var_names(:);
all_var_names = all_var_names + ["_Rw", "_RwX"];
all_var_names = all_var_names';
all_var_names = all_var_names(:);
...
for cnt_sbj = 1:length(subjects_inputs)
    ...
    idx_rew = rewards==1;
    idx_unr = rewards==0;
    rew_unch = inputs_struct.input.inputReward(...
        (3-results_struct.results.choice)'+2*(0:(results_struct.expr.Ntrials-1)));
    idx_rew_unch = rew_unch==1;
    idx_unr_unch = rew_unch==0;
    ...
    for l = 2:150
        ...
        for i_dim=1:6
            ...
            for k = l-1:-1:l-lags_to_fit % start from the previous trial and go back
                ...
                if obj2dim_map(targUnch(k))==obj2dim_map(targets(1,l))
                    Xs_curr_map = [Xs_curr_map ...
                                    -(idx_rew_unch(k)-idx_unr_unch(k))];
                elseif obj2dim_map(targUnch(k))==obj2dim_map(targets(2,l))
                    Xs_curr_map = [Xs_curr_map ...
                                    idx_rew_unch(k)-idx_unr_unch(k)];
                else
                    Xs_curr_map = [Xs_curr_map zeros(1,1)];
