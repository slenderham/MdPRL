function [alpha,exp_r,xp,pxp,bor,g] = bms(lme, families)
    ...
    nf = length(families);
    contrast = zeros(Nk,nf);
    tmp = [];
    for i=1:nf
        indf = families{i};
        assert(~isempty(indf), ['Error: family #',num2str(i),' is empty!']);
        assert(isempty(intersect(tmp,indf)), 'Error: families are not mutually exclusive!')
        tmp = [tmp; full(indf(:))];
        contrast(indf,i) = 1;
    end
    ...
    % prior observations
    %--------------------------------------------------------------------------
    alpha0 = (contrast ./ sum(contrast, 1))*ones(nf, 1)/nf;
    alpha   = alpha0;
    ...
    bor = BMS_bor(lme',posterior,priors,contrast);
