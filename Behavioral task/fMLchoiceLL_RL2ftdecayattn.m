function [loglikehood, V, A] = fMLchoiceLL_RL2ftdecayattn(xpar, sesdata)
%
...
beta_attn_choice = abs(xpar([NparamWithLR+1]));
if sesdata.flagSepAttn==1
    beta_attn_learn = abs(xpar([NparamWithLR+2]));
else
...
for cnt_trial=1:ntrials    
    ...
    if correct
        ...
        v = update(v, idxC, idxW, alpha_rewColor*attn_w_learn(1)) ;

        idxC = idx_shape(choice) ;
        idxW = idx_shape(3-choice) ;
        v = decayV(v, find([1:3]~=idx_shape(choice)), decay) ;
        [idxW, idxC] = idxcouple(idxW, idxC, correct, 0, flag_updatesim) ;
        v = update(v, idxC, idxW, alpha_rewShape*attn_w_learn(2)) ;
        ...
        [idxW, idxC] = idxcouple(idxW, idxC, correct, 0, flag_updatesim) ;
        v = update(v, idxC, idxW, alpha_unrColor*attn_w_learn(1)) ;

        idxW = idx_shape(choice) ;
        idxC = idx_shape(3-choice) ;
        v = decayV(v, find([1:3]~=idx_shape(choice)), decay) ;
        [idxW, idxC] = idxcouple(idxW, idxC, correct, 0, flag_updatesim) ;
        v = update(v, idxC, idxW, alpha_unrShape*attn_w_learn(2)) ;
