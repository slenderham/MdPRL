...
for cnt_probe = 1:length(results.probEst)
    ...
    for cnt_sbj = 1:length(idxperf)
        ...
        curr_subj_vals = all_values{5, 1, idxperf(cnt_sbj)};
        ...
        omega = attns.fit_results{5, 1, idxperf(cnt_sbj)}.params(3);
        beta = attns.fit_results{5, 1, idxperf(cnt_sbj)}.params(2);
...
cmap(6,:) = 0.5;
for i=1:6
    plot(rsqs(:,i), 'Color', cmap(i,:), 'LineWidth', 1);hold on
end
